...
public class AppointmentsController : ApiControllerBase
{
    ...
    public async Task<ActionResult<PagedResult<AppointmentDto>>> Search(
        ...
        => Ok(await _appointmentService.SearchAsync(request, doctorId, patientId, date));

    [HttpGet("{id:int}")]
    public async Task<ActionResult<AppointmentDto>> GetById(int id) => Ok(await _appointmentService.GetByIdAsync(id));
    ...
    {
        var created = await _appointmentService.BookAsync(request);
